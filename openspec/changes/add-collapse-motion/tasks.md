...
- [ ] 1.1 Shared hiding mechanism: replace `display: none` with the animatable
      collapsed state (max-height + opacity + overflow + pointer-events per tab
      row), scoped to `tab-group[zstg-key]`, selected tab excluded
- [ ] 1.2 Preset transition blocks keyed by the `zstg-motion` attribute:
...
- [ ] 1.3 `prefers-reduced-motion: reduce` zeroes every preset's durations

## 2. Script

- [ ] 2.1 Prefs `zen.stg.collapseMotion` (string: off|swift|fold|cascade, default
...
- [ ] 3.1 One radio group of four (Off first, then Swift/Fold/Cascade) with the
...
- [ ] 4.1 verify.ps1: the collapse anchor follows the new rule; anchors for the
