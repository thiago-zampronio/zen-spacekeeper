...
1. **Bump the version in three places** (verify.mjs keeps them honest):
...
2. **Write the version's entry in `CHANGELOG.md`** — newest first, one `## x.y.z`
   heading per version. verify.mjs fails without it, so a release cannot ship
   silent.
...
4. **Refresh the profile and verify**: run the installer (`./install.sh`),
   then `verify.mjs` — EVERYTHING IN SYNC before pushing. Exception: when the
...
5. **Commit, push, publish**: `gh release create vX.Y.Z --latest` with the
   tag created on the bump commit itself — verify.mjs fails any tag whose
